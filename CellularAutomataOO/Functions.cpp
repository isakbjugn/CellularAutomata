...
void randomizeArray(int *arr, int length, int lower, int upper)
{
	for (int i = 0; i < length; i++)
	{
...
{
	std::vector<int> indeces = std::vector<int>(values);
...
{
	std::vector<int> result = std::vector<int>(values);
	std::vector<int> indeces = std::vector<int>(values);
...
{
	std::vector<int> indeces = std::vector<int>(100);

	for (int i = 0; i < 100; i++)
	{
		indeces = randomIndeces(100);
...
bool randomBit()
{
	return bool(randomWithLimits(0, 1));
}
...
double getLowerBound(double fitness)
{
	if (fitness > 0.5)
	{
		// return 1 / (100 * (1 - fitness));
		return 4 * double(std::pow(fitness - 0.5, 3.0));
	}
	return 0;
}